...
"""
test_v3.py — Diagnostics for PPO V3 Agent

Runs a single episode of the trained V3 agent on the base/stationary
environment, printing out specific step-by-step actions and inventory
levels to determine its failure mode.
"""
...
def test_v3_agent():
    ...
    # 3. Load the DummyVecEnv and vec_normalize_v3.pkl
    vec_env = DummyVecEnv([make_env])
    vec_env = VecNormalize.load("data/models/vec_normalize_v3.pkl", vec_env)
    
    # CRITICAL: training=False, norm_reward=False
    vec_env.training = False
    vec_env.norm_reward = False

    # 4. Load the ppo_v3_supply_chain.zip model (device="cpu")
    model = PPO.load("data/models/ppo_v3_supply_chain.zip", env=vec_env, device="cpu")

    # Access the raw un-vectorized environment to read state
    raw_env = vec_env.envs[0].unwrapped

    print("=" * 60)
    print("Starting V3 Diagnostics (1 Episode)")
    print("=" * 60)
